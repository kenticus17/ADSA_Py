...
def U_setup(xlFilePath): 
    wb = load_workbook(xlFilePath)
    sheet = wb.active
    Nrow = len(tuple(sheet.rows))
    U = np.empty((Nrow-1, 2))
    zmax = 0
    zmin = 1e10
...
def initiallize_q0(U, zmax, Sessile, midSliceFactor = 0.1, nrad=50):
...
def run_xlsx_set(xlsxDataFile, resultFile, Sessile):
    global q_global
    wb = load_workbook(xlsxDataFile)
    nsheets = len(wb.worksheets)
    count = 0
    complete = count/nsheets
    for sheet in wb.worksheets:
        ...
        try:
            ...
            ans = q_global[i][:5]
            vol, CA, c, R0, H, SA = Physical_Results(U, ans, resultFile, count, Sessile)
        count += 1 
        complete = count/nsheets
        print(f'{int(complete*100)}%')
    return None
